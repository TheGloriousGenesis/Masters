...
T = 5000
N = 10000 # Made smaller because my pc is slow.
mean = np.zeros(N)
stdv = np.zeros(N)
mu_mock = -1
...
#--------------------
mu_guess =  1
sigma_eta_guess = 0.5 #can't be zero or else f_ht_theta dont work
ht_guess = np.array([np.random.normal() for i in range(T)]) #np.full(T,0.6)
#--------------------
